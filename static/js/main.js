...
(function () {
  ...
  document.querySelectorAll('.translate-btn').forEach(function (btn) {
    btn.addEventListener('click', async function () {
      ...
      try {
        // project/gallery form: data-src → textarea/input name, data-target → target id
        const srcName = btn.dataset.src;
        const srcField = btn.dataset.srcField; // profile form
        const targetId = btn.dataset.target;
        const isKpi = btn.dataset.srcKpi;

        let sourceText = '';
        if (isKpi) {
          const rows = document.querySelectorAll('[name="kpi_item"]');
          sourceText = Array.from(rows).map(r => r.value.trim()).filter(Boolean).join('\n');
        } else if (srcName) {
          const el = document.querySelector('[name="' + srcName + '"]');
          sourceText = el ? el.value.trim() : '';
        } else if (srcField) {
          const el = document.querySelector('[name="' + srcField + '"]');
          sourceText = el ? el.value.trim() : '';
        }

        if (!sourceText) { alert('번역할 원문이 없습니다.'); return; }
        const translated = await translateText(sourceText);

        if (isKpi) {
          // KPI EN rows 채우기
          const enRows = document.getElementById('kpiRowsEn');
          const lines = translated.split('\n').filter(Boolean);
          const existing = enRows.querySelectorAll('[name="kpi_item_en"]');
          lines.forEach((line, i) => {
            if (existing[i]) { existing[i].value = line; }
            else {
              const row = document.createElement('div');
              row.className = 'kpi-row';
              row.innerHTML = '<input type="text" name="kpi_item_en" value="' + line.replace(/"/g, '&quot;') + '" />'
                + '<button type="button" class="kpi-remove-btn" aria-label="삭제">×</button>';
              enRows.appendChild(row);
            }
          });
        } else {
